...
/**
 * Generate a branded A4 PDF report for Taklimakan Desert Monitor.
 *
 * @param {Object} opts
...
export function exportReport({ region, yearA, yearB, stats, timeseries, regionStats }) {
  ...
  pdf.setFontSize(22);
  pdf.text("Taklimakan Desert Monitor", margin, y + 16);
  ...
  pdf.text("Taklimakan Desert Monitor \u2022 Data: Sentinel-2 via Google Earth Engine", margin, y);
  pdf.text("Generated by Guarding the Green Wall Project", pageW - margin, y, { align: "right" });

  pdf.save(`taklimakan-report-${region.replace(/\s+/g, "-").toLowerCase()}.pdf`);
}
